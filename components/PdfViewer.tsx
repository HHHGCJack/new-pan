    ...
    <div className="fixed inset-0 z-[99999] flex flex-col bg-black/95 backdrop-blur-xl h-[100dvh]">
      ...
      <div className={`flex justify-between items-center p-2 md:p-4 shadow-lg z-20 shrink-0 safe-area-top ${isDark ? 'bg-black/90 border-b border-white/20' : 'bg-gray-900 border-b border-gray-800'}`}>
        ...
        <div className="flex items-center space-x-1 md:space-x-2 bg-black/50 rounded-lg p-1 border border-gray-700">
          <button onClick={zoomOut} className={`p-1.5 md:p-2 rounded-md transition-colors flex items-center ${isDark ? 'text-white hover:bg-white/20' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`} title="缩小">
            <ZoomOut size={18} className="md:w-5 md:h-5" />
          </button>
          <span className={`text-xs w-8 text-center ${isDark ? 'text-gray-300' : 'text-gray-400'}`}>
            {Math.round(scale * 100)}%
          </span>
          <button onClick={zoomIn} className={`p-1.5 md:p-2 rounded-md transition-colors flex items-center ${isDark ? 'text-white hover:bg-white/20' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`} title="放大">
            <ZoomIn size={18} className="md:w-5 md:h-5" />
          </button>
          <button onClick={resetZoom} className={`p-1.5 md:p-2 rounded-md transition-colors flex items-center ${isDark ? 'text-white hover:bg-white/20' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`} title="重置缩放">
            <Maximize size={18} className="md:w-5 md:h-5" />
          </button>
          <div className="w-px h-6 bg-gray-700 mx-1"></div>
          <button onClick={handleDownload} className={`p-1.5 md:p-2 rounded-md transition-colors flex items-center ${isDark ? 'text-white hover:bg-white/20' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`} title="下载 PDF">
